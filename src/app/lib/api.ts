...
// Helper para convertir los datos del frontend al formato que espera el backend (Base de Datos)
const toBackendService = (datos: any) => {
  const backendType = datos.type === 'preventive' ? 'preventivo' :
                     datos.type === 'installation' ? 'instalacion' :
                     datos.type === 'corrective' ? 'correctivo' : 'otro';

  return withNulls({
    nombre: datos.name,
    tipo: backendType,
    prioridad: datos.priority,
    estado: datos.status === 'completed' ? 'completado' : datos.status === 'in-progress' ? 'en progreso' : 'pendiente',
    fecha_inicio: datos.startDate,
    fecha_fin: datos.endDate,
    fecha_estimada: datos.estimatedCompletionDate,
    nombre_solicitante: datos.requesterName,
    telefono_solicitante: datos.requesterPhone,
    email_solicitante: datos.requesterEmail,
    area_solicitante: datos.requesterArea,
    tecnico_asignado: datos.assignedTechnician,
...
export const authApi = {
  login: async (email: string, password: string) => {
    ...
    // para que el interceptor lo pueda usar automáticamente.
...
export const solicitanteApi = {
  ...
  create: async (datos: any) => {
    const response = await api.post('/api/solicitantes', datos);
    return response.data;
  },
  update: async (id: string, datos: any) => {
    const response = await api.put(`/api/solicitantes/${id}`, datos);
    return response.data;
  },
  remove: async (id: string) => {
    const response = await api.delete(`/api/solicitantes/${id}`);
    return response.data;
  },
};

// --- Servicios de Taller (Carpenter) ---
export const serviceApi = {
  ...
  // Métodos de actualización parcial (PATCH)
  ...
  cambiarStatus: async (id: string, status: ServiceStatus) => {
    const response = await api.patch(`/api/servicios/${id}/status`, { status });
    return normalizeService(response.data);
  },

  cambiarPrioridad: async (id: string, prioridad: ServiceRequest['priority']) => {
    const response = await api.patch(`/api/servicios/${id}/prioridad`, { prioridad });
    return normalizeService(response.data);
  },

  // Gestión de Utensilios dentro de un servicio
